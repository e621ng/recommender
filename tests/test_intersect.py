...
from recommender.api.engine import intersect_top


def test_disjoint_lists():
    a = [(1, 1.0), (3, 0.5)]
    b = [(2, 1.0), (4, 0.5)]
    assert intersect_top(a, b, m=6) == []


def test_full_overlap_uses_min_weight():
    a = [(1, 0.8), (2, 1.2)]
    b = [(1, 0.6), (2, 1.5)]
    result = intersect_top(a, b, m=6)
    assert len(result) == 2
...
def test_result_capped_at_m():
    ...
    assert len(intersect_top(a, b, m=3)) == 3
...
def test_empty_a():
    assert intersect_top([], [(1, 1.0)], m=6) == []
...
def test_both_empty():
    assert intersect_top([], [], m=6) == []


def test_single_element_overlap():
    result = intersect_top([(7, 0.5)], [(7, 0.5)], m=6)
    assert len(result) == 1
    assert result[0][0] == 7
...
def test_single_element_no_overlap():
    assert intersect_top([(1, 0.5)], [(2, 0.5)], m=6) == []


def test_m_zero_returns_empty():
    a = [(1, 1.0), (2, 0.5)]
    b = [(1, 1.0), (2, 0.5)]
    assert intersect_top(a, b, m=0) == []
